...
export async function classifyAndSend(summary: string, type: "Product" | "Team" | "Other", link: string): Promise<{ success: boolean; message: string }> {
  try {
    ...
    if (type === 'Product') {
      ...
      // If Team -> handle this on the client side
      console.log('Summary classified as Team. EmailJS will be handled client-side.');
      return { success: true, message: 'Summary classified as Team. EmailJS will be handled client-side.' };
